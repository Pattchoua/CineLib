...
const HeroSectionMD = () => {
    ...
    try {
      ...
      // Set the hero image URL to the backdrop path of the first trending movie
        ...
        setHeroImage(`https://image.tmdb.org/t/p/original${response.data.results[7].backdrop_path}`);
        }
        } catch (error) {
    ...
    <div className='w-full mx-auto p-4'>
        <div className='bg-cover rounded-lg bg-center h-[350px] relative p-4'  style={{ backgroundImage: `url(${heroImage})` }}>
            <div className= 'absolute top-0 right-2 bottom-0 left-0 text-gray-200 w-full h-full bg-black/40 max-h-[700px] flex flex-col justify-center'>
                <h1 className='px-4 text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold'>Manage Your Movie</h1>
                <h3 className='px-4 text-2xl sm:text-3xl  text-cyan-400 md:text-4xl lg:text-5xl font-bold'>Explore In-Depth Movie Details</h3> 
